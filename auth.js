...
function injectAuthScreen() {
    ...
    // Nouvel appareil (aucune trace d'un compte précédent) → Inscription
    // en premier, puisque quelqu'un qui arrive pour la première fois n'a
    // logiquement pas encore de compte. Un appareil qui a déjà servi
    // (token présent, même expiré) → Connexion en premier, il a juste
    // besoin de se reconnecter.
    const userKey = (typeof CONFIG !== 'undefined' && CONFIG.STORAGE_KEYS && CONFIG.STORAGE_KEYS.USER)
        ? CONFIG.STORAGE_KEYS.USER : 'user';
    const hasPriorAccount = !!localStorage.getItem(userKey);
    const loginFirst = hasPriorAccount;
    ...
    authScreen.innerHTML = `
        <div class="auth-container">
            <div class="auth-card">
                ...
                <!-- Connexion -->
                <div id="login-form" class="auth-form" style="display: ${loginFirst ? 'block' : 'none'};">
                    <h2>Connexion</h2>
                ...
                <!-- Inscription -->
                <div id="register-form" class="auth-form" style="display: ${loginFirst ? 'none' : 'block'};">
                    <h2>Inscription</h2>
                    ...
                    <form onsubmit="handleRegister(event)">
                        ...
                        <div class="form-group">
                            ...
                            <input type="tel" id="register-phone" required placeholder="Ex: 0991234567" pattern="[0-9]{9,10}" title="9 ou 10 chiffres, sans espace ni tiret">
                        </div>
                        <div class="form-group" style="display:flex;align-items:flex-start;gap:8px;">
                            <input type="checkbox" id="register-wants-contact" style="margin-top:4px;width:auto;">
                            <label for="register-wants-contact" style="font-size:13px;font-weight:normal;">
                                Souhaitez-vous être contacté(e) par le service technique de Kivirafacile ?
                            </label>
                        </div>
                        <div class="form-group">
                            <label>Mot de passe</label>
...
async function checkAuth() {
    ...
    if (isAuthenticated()) {
        try {
            ...
            // 🆕 Resynchroniser le flag 'userPremium' à CHAQUE vérification, pas
            // seulement au moment du paiement. Sans ça, ce flag reste bloqué sur
            // 'true' pour toujours dès qu'un paiement a été fait un jour — même
            // après qu'un admin ait retiré le premium — et kivira-lessons.js
            // continue de faire confiance à cette valeur périmée en priorité.
            localStorage.setItem('userPremium', userData.isPremium ? 'true' : 'false');
            ...
            // 🆕 Si on vient tout juste de s'inscrire (marqueur posé avant le
            // rechargement dans handleRegister), proposer les notifications
            // maintenant — une seule fois, jamais aux connexions suivantes.
            if (localStorage.getItem('kivira_just_registered') === 'true') {
                localStorage.removeItem('kivira_just_registered');
                if (typeof showNotificationPermissionPrompt === 'function') {
                    setTimeout(showNotificationPermissionPrompt, 800);
                }
            }
...
async function handleRegister(event) {
    ...
    const wantsContact = document.getElementById('register-wants-contact').checked;
    ...
    try {
        ...
        const data = await apiRequest('/auth/register', {
            method: 'POST',
            body: JSON.stringify({ name, email, phone, wantsContact, nick })
        });

        console.log('✅ Inscription réussie');
        saveUserData(data.token, data.user);
        // Le rechargement qui suit efface tout — on note qu'il faut proposer
        // les notifications juste après, une fois la page rechargée.
        localStorage.setItem('kivira_just_registered', 'true');
        window.location.reload();
...
function logout() {
    ...
    localStorage.removeItem('userPremium'); // sinon kivira-lessons.js reste persuadé que l'accès est encore premium
    currentUser = null;
    window.location.reload();
}
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// VÉRIFICATION AUTOMATIQUE DU STATUT (sans dépendre d'un F5 manuel)
//
// Avant : le statut (premium validé, compte bloqué...) n'était relu
// que si l'utilisateur actualisait la page lui-même. Quelqu'un qui
// reste sur l'app sans jamais recharger ne voyait jamais un paiement
// approuvé, ni un blocage.
//
// Maintenant : on revérifie automatiquement (a) à intervalle régulier
// et (b) dès que l'utilisateur revient sur l'onglet (cas le plus
// fréquent : il part payer sur WhatsApp/USSD puis revient).
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

const AUTH_POLL_INTERVAL_MS = 5 * 60 * 1000; // toutes les 5 minutes
let authPollTimer = null;

function startAuthPolling() {
    if (authPollTimer) return; // déjà démarré
...
// Vérification immédiate dès que l'utilisateur revient sur l'onglet
// (ex: retour depuis l'app Mobile Money après un paiement)
...
// Démarrer le sondage une fois l'app chargée
document.addEventListener('DOMContentLoaded', startAuthPolling);
