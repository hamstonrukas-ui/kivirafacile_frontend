...
// ── Afficher le proverbe du jour directement dans l'app ──
async function loadProverbOfTheDay(targetElementId) {
    const el = document.getElementById(targetElementId);
    if (!el) return;
    try {
        const res = await fetch(`${NOTIF_API_BASE}/notifications/proverb-of-the-day`);
        const data = await res.json();
        if (data.success && data.proverb) {
            el.innerHTML = `
                <div style="font-size:13px;opacity:0.8;margin-bottom:4px">💬 Proverbe du jour</div>
                <div style="font-size:16px;font-weight:700">${data.proverb.kivira}</div>
                <div style="font-size:14px;opacity:0.9">${data.proverb.french}</div>
            `;
        }
    } catch (err) {
        console.warn('Proverbe du jour non disponible:', err.message);
    }
}
...
window.loadProverbOfTheDay = loadProverbOfTheDay;
