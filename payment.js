                ...
                <div class="payment-instructions-box">
                    ...
                    <div class="payment-number-box">
                        ...
                        <div>
                            ...
                            <div class="operator-number" id="airtel-number">+243 97 303 7477</div>
                            <div class="operator-account-name" id="airtel-account-name" style="font-size:12px;color:#6b7280;">Zihindula Baltus</div>
                        </div>
                    ...
                    <div class="payment-number-box">
                        ...
                        <div>
                            ...
                            <div class="operator-account-name" id="orange-account-name" style="font-size:12px;color:#6b7280;">Sauveur Mulopwe</div>
                        </div>
                    ...
                    <div class="payment-number-box">
                        ...
                        <div>
                            ...
                            <div class="operator-account-name" id="mpesa-account-name" style="font-size:12px;color:#6b7280;">Alexis</div>
                        </div>
...
async function loadPaymentInstructions() {
    try {
        ...
        if (paymentInstructions) {
            ...
            if (paymentInstructions.airtelAccountName) document.getElementById('airtel-account-name').textContent = paymentInstructions.airtelAccountName;
            if (paymentInstructions.orangeAccountName) document.getElementById('orange-account-name').textContent = paymentInstructions.orangeAccountName;
            if (paymentInstructions.mpesaAccountName) document.getElementById('mpesa-account-name').textContent = paymentInstructions.mpesaAccountName;
        }
    } catch (error) {
...
const TRANSACTION_ID_REGEX = /^[A-Z]{2}\d{6}\.\d{4}\.[A-Z]\d{5}$/i;

function validateTransactionIdFormat(rawId) {
    let id = (rawId || '').trim().toUpperCase();
    // Certains utilisateurs collent le texte complet du SMS avec un préfixe
    // ("TID: ...", "Réf: ...") — on le retire avant de valider le format,
    // comme le fait déjà validateTransactionIdFormatT côté traduction.
    id = id.replace(/^(TID|REF|REFERENCE|R[EÉ]F[EÉ]RENCE|TRANSACTION|ID)\s*:\s*/i, '').trim();
    if (!TRANSACTION_ID_REGEX.test(id)) {
        return {
            valid: false,
...
// Préfixes réseau en RDC — indicatif seulement (la portabilité des numéros
// rend ça imparfait), sert uniquement à avertir, jamais à bloquer le compte.
...
async function submitTransaction(event) {
    ...
    const operatorRaw = document.getElementById('payment-operator').value;
    const operator = (operatorRaw || '').toLowerCase(); // normaliser : le select peut renvoyer "Airtel", pas "airtel"
    ...
    // Validation immédiate côté client — pas de blocage, juste un message clair
    ...
    if (phoneCheck.detectedOperator && phoneCheck.detectedOperator !== operator) {
        const msg = `Ce numéro ressemble à un numéro ${OPERATOR_LABELS[phoneCheck.detectedOperator]}, mais vous avez sélectionné ${OPERATOR_LABELS[operator] || operatorRaw}. Vérifiez votre numéro ou l'opérateur sélectionné.`;
    ...
    try {
        ...
        document.getElementById('submitted-transaction-id').textContent = idCheck.normalized;
    ...
    // 🔓 Débloquer IMMÉDIATEMENT les leçons premium (catégories + badges + clic)
    // Sans ceci, kivira-lessons.js garde ses anciennes cartes verrouillées
    // (canAccess capturé au moment du rendu précédent) et re-propose le paiement.
    window.dispatchEvent(new CustomEvent('premiumStatusChanged'));
    if (window.KiviraLessons && typeof window.KiviraLessons.refresh === 'function') {
