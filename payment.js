...
async function submitTransaction(event) {
    ...
    if (!idCheck.valid) {
        errorDiv.textContent = idCheck.error;
    ...
    if (!phoneCheck.valid) {
        errorDiv.textContent = phoneCheck.error;
        errorDiv.style.display = 'block';
        return;
    }

    if (phoneCheck.detectedOperator && phoneCheck.detectedOperator !== operator) {
        errorDiv.textContent = `Ce numéro ressemble à un numéro ${OPERATOR_LABELS[phoneCheck.detectedOperator]}, mais vous avez sélectionné ${OPERATOR_LABELS[operator] || operator}. Vérifiez votre numéro ou l'opérateur sélectionné.`;
        errorDiv.style.display = 'block';
        return;
    }
