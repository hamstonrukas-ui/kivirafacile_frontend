...
async function loadTranslationPaymentNumbers() {
    ...
    try {
        ...
        container.innerHTML = [
            { name: 'Airtel Money', num: config.airtelNumber, account: config.airtelAccountName },
            { name: 'Orange Money', num: config.orangeNumber, account: config.orangeAccountName },
            { name: 'M-Pesa', num: config.mpesaNumber, account: config.mpesaAccountName }
        ].map(op => `
            <div class="operator-number">
                <span class="op-name">📱 ${op.name}</span>
                <span class="op-num">${op.num}</span>
                ${op.account ? `<span class="op-account" style="display:block;font-size:12px;color:#6b7280;">${op.account}</span>` : ''}
                <button class="btn-copy" onclick="copyTranslationNumber('${op.num}')">📋 Copier</button>
            </div>
        `).join('');
    } catch (err) {
        container.innerHTML = `
            <div class="operator-number"><span class="op-name">📱 Airtel Money</span><span class="op-num">+243 97 303 7477</span><span class="op-account" style="display:block;font-size:12px;color:#6b7280;">Zihindula Baltus</span></div>
            <div class="operator-number"><span class="op-name">📱 Orange Money</span><span class="op-num">+243 85 421 7357</span><span class="op-account" style="display:block;font-size:12px;color:#6b7280;">Sauveur Mulopwe</span></div>
            <div class="operator-number"><span class="op-name">📱 M-Pesa</span><span class="op-num">+243 83 442 4452</span><span class="op-account" style="display:block;font-size:12px;color:#6b7280;">Alexis</span></div>
        `;
    }
}
